...
function eventsProtocolFactory (
    ...
    EventsProtocol.prototype.publishTaskNotification = function (taskId, data) {
        ...
        // message.status has to be one of ['completed', 'failed', 'ongoing']
        var statusEnum = ['completed', 'failed', 'inProgress'];
        if (!_.includes(statusEnum, data.status)) {
